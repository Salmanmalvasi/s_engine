...
def build_svg(title, entries, filename):
    ...
    content.append(f'  <text x="{width/2}" y="{height-20}" text-anchor="middle" class="label">String Length (n)</text>')
    ...
    for step in range(5):
        ...
        content.append(f'  <text x="{margin-10}" y="{y+4}" text-anchor="end" class="label">{label_val:.4f}</text>')
    ...
    for n, time in entries:
        ...
        poly.append(f"{x},{y}")
    points = " ".join(poly)
    content.append(f'  <polyline fill="none" stroke="#2ca02c" stroke-width="2" points="{points}" />')
    for n, time in entries:
        x = margin + normalize(n, x_min, x_max, plot_width)
        y = height - margin - normalize(time, y_min, y_max, plot_height)
        content.append(f'  <circle cx="{x}" cy="{y}" r="4" fill="#2ca02c" />')

    svg = SVG_TEMPLATE.format(width=width, height=height, content="\n".join(content))
    (OUTPUT_DIR / filename).write_text(svg)

build_svg("LCS Time Complexity (Random Inputs)", data, "lcs_timing.svg")
...
<body>
  <h1>Longest Common Subsequence Timing</h1>
  <section>
    <h2>Timing Graph</h2>
    <object type="image/svg+xml" data="lcs_timing.svg" width="900" height="500"></object>
    <p><a href="lcs_timing.svg" target="_blank">Open chart in new tab</a></p>
  </section>
  <section>
    <h2>Data Table</h2>
    <table>
      <tr><th>n (string length)</th><th>Time (seconds)</th></tr>
"""
for n, t in data:
