...
def build_svg(title, entries, filename):
    ...
    content.append(f'  <text x="{width/2}" y="{height-20}" text-anchor="middle" class="label">Rod Length (n)</text>')
    ...
    for step in range(5):
        ...
        content.append(f'  <text x="{margin-10}" y="{y+4}" text-anchor="end" class="label">{label_val:.6f}</text>')
    ...
    for n, time in entries:
        ...
        poly.append(f"{x},{y}")
    points = " ".join(poly)
    content.append(f'  <polyline fill="none" stroke="#d62728" stroke-width="2" points="{points}" />')
    for n, time in entries:
        x = margin + normalize(n, x_min, x_max, plot_width)
        y = height - margin - normalize(time, y_min, y_max, plot_height)
        content.append(f'  <circle cx="{x}" cy="{y}" r="4" fill="#d62728" />')

    svg = SVG_TEMPLATE.format(width=width, height=height, content="\n".join(content))
    (OUTPUT_DIR / filename).write_text(svg)

if data:
    build_svg("Rod Cutting Time Complexity (O(n^2))", data, "rod_cutting_timing.svg")
...
<head>
  <meta charset="UTF-8">
  <title>Rod Cutting Charts</title>
  <style>
...
<body>
  <h1>Rod Cutting Timing Analysis</h1>
  <section>
    <h2>Timing Graph (O(n²))</h2>
    <object type="image/svg+xml" data="rod_cutting_timing.svg" width="900" height="500"></object>
    <p><a href="rod_cutting_timing.svg" target="_blank">Open chart in new tab</a></p>
  </section>
  <section>
    <h2>Data Table</h2>
    <table>
      <tr><th>Rod Length (n)</th><th>Time (seconds)</th></tr>
"""
for n, t in data:
    html_content += f"      <tr><td>{n}</td><td>{t:.6f}</td></tr>\n"
html_content += """    </table>
  </section>
</body>
</html>
