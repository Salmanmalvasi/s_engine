...
def build_svg(title, entries, filename):
    ...
    content.append(f'  <text x="{width/2}" y="{height-20}" text-anchor="middle" class="label">Number of Symbols (n)</text>')
    ...
    for n, time in entries:
        ...
        poly.append(f"{x},{y}")
    points = " ".join(poly)
    content.append(f'  <polyline fill="none" stroke="#ff7f0e" stroke-width="2" points="{points}" />')
    for n, time in entries:
        x = margin + normalize(n, x_min, x_max, plot_width)
        y = height - margin - normalize(time, y_min, y_max, plot_height)
        content.append(f'  <circle cx="{x}" cy="{y}" r="4" fill="#ff7f0e" />')

    svg = SVG_TEMPLATE.format(width=width, height=height, content="\n".join(content))
    (OUTPUT_DIR / filename).write_text(svg)

build_svg("Huffman Coding Time Complexity", data, "huffman_timing.svg")
...
<head>
  <meta charset="UTF-8">
  <title>Huffman Coding Charts</title>
  <style>
...
<body>
  <h1>Huffman Coding Timing Analysis</h1>
  <section>
    <h2>Timing Graph</h2>
    <object type="image/svg+xml" data="huffman_timing.svg" width="900" height="500"></object>
    <p><a href="huffman_timing.svg" target="_blank">Open chart in new tab</a></p>
  </section>
  <section>
    <h2>Data Table</h2>
    <table>
      <tr><th>n (symbols)</th><th>Time (seconds)</th></tr>
"""
for n, t in data:
